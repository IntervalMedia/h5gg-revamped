...
|---|---|---|---|---|---|
| `setButtonImage(url)`<br>**Stable** | Image URL string. | `boolean`. | Downloads image data and updates the local floating button. Download failure resolves `false`. Data larger than the shared GlobalView buffer still resolves `true`, but is not copied to that buffer. | `await setButtonImage("https://example.test/icon.png");` | `initFloatMenu` block in `Tweak.mm` |
